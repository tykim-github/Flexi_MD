...
uint8_t BSP_SD_Init(void)
{
     ...
     hsd1.Init.BusWide = SDMMC_BUS_WIDE_1B;									     // 1bit mode init. for micro-sd
	 ...
	 if (BSP_SD_IsDetected() != SD_PRESENT)										// Card detection check if sd card is inserted or not
	 {
	  ...
	  sd_state = HAL_SD_Init(&hsd1);      // SD Card init.

	  // Configure SD Bus width (4 bits mode selected)

	  hsd1.Init.BusWide = SDMMC_BUS_WIDE_4B;									    // 4-bit mode bus operation
